...
export async function PATCH(request: Request, context: RouteContext<"/api/theatre/plays/[id]">) {
  try {
    ...
    await prisma.$transaction(async tx=>{
      const status = s("status") === "UPCOMING" ? "UPCOMING" : s("status") === "DRAFT" ? "DRAFT" : "PUBLISHED";
      await tx.play.update({ where:{id:play.id}, data:{title:s("title"),abstract:s("abstract"),directorialNote:s("directorialNote"),description:s("description"),coverImage:s("coverImage")||null,duration:s("duration")?Number(s("duration")):null,launchedOn:s("launchedOn")?new Date(s("launchedOn")):null,endedOn:s("endedOn")?new Date(s("endedOn")):null,status,isFeatured:d.get("isFeatured")==="on",updated:now} });
      if (d.has("director") || d.has("onStage") || d.has("offStage")) {
      await Promise.all([
        ...(d.has("director") ? [tx.playMaker.deleteMany({where:{playId:play.id}})] : []),
        ...(d.has("onStage") ? [tx.playCast.deleteMany({where:{playId:play.id}})] : []),
        ...(d.has("offStage") ? [tx.playCrew.deleteMany({where:{playId:play.id}})] : []),
      ]);
